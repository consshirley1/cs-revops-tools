...
<style>
    ...
    .provider-name {{ font-size: 1.25rem; font-weight: 500; color: {DARK}; }}
    .provider-type {{ font-size: 0.85rem; color: {TEAL_MID}; margin-top: 0.1rem; }}
    .flag-present {{ color: {TEAL_DARK}; font-weight: 500; }}
    .flag-absent  {{ color: #9ca3af; }}
    .tag-blue   {{ background:{TEAL_LIGHT}; color:{TEAL_DARK}; font-size:0.72rem; padding:0.2rem 0.6rem; border-radius:3px; }}
    .tag-gray   {{ background:#f3f4f6; color:#6b7280; font-size:0.72rem; padding:0.2rem 0.6rem; border-radius:3px; }}
    .tag-green  {{ background:{TEAL_LIGHT}; color:{TEAL_DARK}; font-size:0.72rem; padding:0.2rem 0.6rem; border-radius:3px; }}
...
if run and domain_input:
    ...
    if error:
        ...
        conf_tag = {
            "High":   f'<span class="tag-green">High Confidence</span>',
            "Medium": f'<span class="tag-yellow">Medium Confidence</span>',
            "Low":    f'<span class="tag-gray">Low Confidence</span>',
        ...
        with col2:
            ...
            st.markdown(f"<div style='font-size:0.85rem; color:{DARK};'>{provider['crm']}</div>", unsafe_allow_html=True)
        with col3:
            st.markdown('<div class="section-label">Typical Segment</div>', unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:0.85rem; color:{DARK};'>{provider['segment']}</div>", unsafe_allow_html=True)

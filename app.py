...
def home_page():
    ...
    with col_stats:
        ...
        with s2:
            st.markdown(f'<div class="stat-box"><div class="stat-num">7</div><div class="stat-label">Production<br>Apps Shipped</div></div>', unsafe_allow_html=True)
            st.markdown("<div style='height:0.6rem'></div>", unsafe_allow_html=True)
    ...
    with a3:
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">✉️ Get In Touch</div>
...
pg = st.navigation(
    {
        ...
        "Tools": [
            ...
            st.Page("pages/7_Lead_Researcher.py",     title="Lead Researcher",    icon="🔬"),
            st.Page("pages/8_SFDC_Account_Mapper.py", title="White Space Mapper", icon="📈"),
            st.Page("pages/9_SFDC_Perm.py",           title="Permission Auditor", icon="🔐"),
            st.Page("pages/10_SOQL_to_Human.py",      title="SOQL Translator",    icon="🔍"),
        ],
        "Projects": [
